...
// Specify workers debugger
const debug = util.debuglog("workers");

// Instantiate the workers object
const workers = {};

// Lookup all the checks, gather their data, send to validator
...
workers.validateCheckData = function (originalCheckData) {
  ...
  originalCheckData.checkId =
    ...
    originalCheckData.checkId.trim().length == config.checkRandomNumber
  ...
  // Set the keys that may not been set (if the workers have never seen this check before)
  ...
  _data.update("checks", newCheckData.checkId, newCheckData, function (err) {
    if (!err) {
      ...
      if (alertWarranted) {
        ...
        debug("Check outcome has not been changed, no alert needed");
      }
    } else {
...
workers.alertUserToStatusChange = function (newCheckData) {
  ...
  const phone = newCheckData.userPhone;
  helpers.twilio.sendSms(phone, msg, function (err) {
    if (!err) {
      debug(
        "Success: User was alerted to a state change in their check, via SMS: " +
          msg
      );
    } else {
      debug(
...
workers.logRotationLoop = function () {
  setInterval(function () {
    workers.rotateLogs();
  }, config.loopLogRotationWorkerInterval);
};
